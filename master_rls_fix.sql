-- Enable RLS on all tables to be safe
...
-- Drop any old or broken policies safely
DROP POLICY IF EXISTS "Allow authenticated full access on members" ON members;
DROP POLICY IF EXISTS "Allow authenticated full access on memberships" ON memberships;
DROP POLICY IF EXISTS "Allow authenticated full access on membership_plans" ON membership_plans;
DROP POLICY IF EXISTS "Allow authenticated full access on payments" ON payments;
DROP POLICY IF EXISTS "Allow authenticated full access on pt_assignments" ON pt_assignments;
DROP POLICY IF EXISTS "Allow authenticated full access on pt_packages" ON pt_packages;
DROP POLICY IF EXISTS "Allow authenticated full access on attendance_logs" ON attendance_logs;
DROP POLICY IF EXISTS "Allow authenticated full access on whatsapp_logs" ON whatsapp_logs;

-- CREATE FULL ACCESS POLICIES FOR ALL AUTHENTICATED STAFF/OWNERS
