...
export function LiveFeed() {
  ...
  return (
    <Card className="bg-card border-border rounded-xl h-full flex flex-col">
    <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-border/50">
        <div className="flex items-center gap-3">
          <CardTitle className="text-base font-bold text-foreground">Live attendance feed</CardTitle>
        ...
        <div className="h-[400px] overflow-y-auto divide-y divide-dark-600 custom-scrollbar">
          {feed.length === 0 ? (
            ...
            feed.map((log) => (
              <div 
                key={log.id} 
                className="flex items-center justify-between p-4 hover:bg-muted/50 transition-colors animate-in fade-in slide-in-from-top-2"
              >
                <div className="flex items-center gap-4">
                  {/* Avatar fallback */}
                  <div className="h-10 w-10 shrink-0 rounded-full bg-muted flex items-center justify-center text-gold-500 font-bold border border-border">
                    {log.members?.full_name?.charAt(0).toUpperCase() || '?'}
                  </div>
                  
                  <div className="min-w-0">
                    <p className="text-foreground font-medium text-sm truncate max-w-[120px] sm:max-w-[200px]">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Badge variant="outline" className="text-[10px] hidden sm:inline-flex bg-muted text-muted-foreground border-border">
